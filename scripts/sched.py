...
def main():
    ...
    #if args.action == 'profile' and args.end > 30:
    #    parser.error('profile with -e > 30?')

    config = args.config
    if args.config == ['ALL']:
        config = [ 'serial', 'omp' ]
    if 'omp' in config and not args.wait:
        parser.error('must define omp policy with omp config')
    ...
    experiments = {
            'serial': {
                ...
                'refine': {'instrument': [''] },
                'pinfi':  {'instrument': [''] },
                'pinfi-detach':  {'instrument': [''] },
                'small' : { 'nthreads' : [''] },
                },
            'omp': {
                'inputs':[ 'small', 'large' ],
                'golden': { 'instrument': [''] },
                'safire': { 'instrument': ['all', 'app', 'omplib'] },
                'refine': { 'instrument': ['all', 'app', 'omplib'] },
                'pinfi':  { 'instrument': ['all', 'app', 'omplib'] },
                'pinfi-detach':  { 'instrument': ['all', 'app', 'omplib'] },
    ...
    for t in args.tools:
        for c in config:
            # If config is serial wait must be empty
            if c == 'serial':
                wait = ''
            else:
                wait = args.wait
            for i in experiments[c]['inputs']:
                # XXX: do large if there's time for PINFI
                #if ( t == 'pinfi' or t == 'pinfi-detach' or t == 'refine' ) and i =='large':
                if ( t == 'pinfi' or t == 'refine' ) and i =='large':
                    # Skip large experiments for PINFI
                    continue
                for n in experiments[c][i]['nthreads']:
                    for ins in experiments[c][t]['instrument']:
                        # Skip thread target FI when nthreads == 1
                        if (args.action == 'fi-0' or args.action == 'fi-1-15') and (n=='' or int(n) == 1):
                            continue
                        # Skip thread target FI when ins is *NOT* omplib
                        if (args.action == 'fi-0' or args.action == 'fi-1-15') and (ins != 'omplib'):
                            continue
                        #print( 'Experiment: %s %s %s %s %s %s %s %s %s [%s]'%( t, args.action, c, wait, n, ins, i, args.start, args.end, ', '.join(args.apps) ) )
                        exps = check(args.appdir, args.resdir, t, c, wait, args.apps, args.action, ins, n, i, args.start, args.end, args.generate)
                        if exps:
                            print('==== EXPERIMENT ====')
                            print( 'Experiment: %s %s %s %s %s %s %s %s %s [%s]'%( t, args.action, c, wait, n, ins, i, args.start, args.end, ', '.join(args.apps) ) )
                            #print(exps)
                            ...
                            if args.generate:
                                ...
                                generate_jobs(args.nodes, args.partition, args.timelimit, exps, c, wait, t, args.action, ins, n, i, args.start, args.end )
